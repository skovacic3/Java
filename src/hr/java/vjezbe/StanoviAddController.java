...
public class StanoviAddController {
	...
	public void stisnutGumb() throws IOException {
		...
		try {
			tempKvadratura = Integer.parseInt(unosKvadraturaId.getText());
		}
		catch(Exception e) {
			System.out.println("");
		}
		...
		if(validate()) {
			allItems.add(new Stan(tempNaziv, tempOpis, tempCijena, tempStanje, tempKvadratura, maxId.getAsLong() + 1 ));
	...
	public boolean validate() {
        ...
        if (unosKvadraturaId.getText().trim().isEmpty()) {
